...
| `reto.html` | Chat conversacional en una sola página. Guion `STEPS` de 14 pasos (bienvenida → experimento 2h → el estado decide → entorno tóxico/contrario → Mundo A/B → orden del cambio → resumen → recién aquí WHAPE → 31-dic → cierre → fork); cada paso `{bot[], ask?, opts[], render?, fork?}`; `go(i)` reproduce con typing. **Lee `whape_h`** de localStorage (`MYH`) → saludo personalizado con las horas que el usuario calculó en index; fallback genérico si llega directo. **Reglas de copy: SIEMPRE 2-3 botones y cada botón responde naturalmente la pregunta del bot.** `focus:"premium"` SOLO en opciones con alternativa neutral (así `FOCUS` refleja intención real) → decide orden de tarjetas y etiqueta "⭐ Recomendado". Renders: `contrast` (Mundo A/B) y `summary` (3 descubrimientos). Fork con **oferta estilo Hormozi**: anclaje en burbujas (montarlo solo >S/1,000 vs S/99) + tarjeta Premium con stack de valor (`.stack` S/297+499+97+147 = S/1,040 tachado → `.now` S/99), bonos, escasez con razón (`.scar` 100 cupos), **garantía 7 días** (`.guar`); tarjeta Gratis = downsell condicional ("¿Aún con dudas?" solo si premHot). Premium → wa.me `(premium)` / Gratis → wa.me `(gratis)`. `DAYS` dinámico hasta 31-dic |
